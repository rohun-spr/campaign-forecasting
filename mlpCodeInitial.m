function mlpCodeInitial()

TAG = 'TW_PROB_PORTION_LOGSIG_PURELIN_14th';
...
for lr = [0.05]
    for momentum = [0.6]
        ...
        X = 1:length(XTRAIN);
        plot(X,YTRAIN,'--go',X,YFITTRAIN',':b*')
        ...
        prediction_file_name = 'weighted_mean/testMLP.csv';
        fid=fopen(prediction_file_name,'w');
        for idx = 1:length(YTEST)
            actual = YTEST(idx);
            prediction = YTESTFIT(idx);
...
%     YTESTFIT = mean(z,3);
%     printForMLP(size, lr, momentum, YTRAIN, YFITTRAIN, XTRAIN, YTEST, YTESTFIT, XTEST, errorOutFile, net, TAG)
